...
export class TodoView {
  ...
  showForm(form: HTMLFormElement, btn: HTMLButtonElement) {
    form.classList.remove('hidden');
    btn.parentElement?.classList.add('hidden');
  }

  hideForm(form: HTMLFormElement, btn: HTMLButtonElement) {
  ...
  renderTodos(
    ...
    todos.forEach(todo => {
      ...
      btnEdit.addEventListener('click', () =>
        this.renderEditForm(li, todo, onEdit),
      );
  ...
  private renderEditForm(
    ...
    const editTodoFormClass: string = 'edit-todo-form';

    const getNewTodoText = (inputEl: HTMLInputElement) => inputEl.value.trim();
